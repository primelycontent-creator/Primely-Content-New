...
export async function POST(req: Request) {
  try {
    ...
    const profile = await prisma.creatorProfile.upsert({
      ...
      update: { profileImageAssetId: asset.id },
      create: { userId: auth.userId, profileImageAssetId: asset.id },
      include: {
        profileImageAsset: true,
      },
    });

    return NextResponse.json({ ok: true, asset, profile });
  } catch (e: any) {
    console.error("api/creator/profile/profile-image POST error:", e);
