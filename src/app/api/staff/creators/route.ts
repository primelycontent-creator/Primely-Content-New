...
import { CreatorApprovalStatus, CreatorWorkMode } from "@prisma/client";
...
function isWorkMode(v: string): v is CreatorWorkMode {
  return v === CreatorWorkMode.FULL_TIME || v === CreatorWorkMode.PART_TIME;
}

function isApprovalStatus(v: string): v is CreatorApprovalStatus {
  return (
    v === CreatorApprovalStatus.PENDING ||
    v === CreatorApprovalStatus.APPROVED ||
    v === CreatorApprovalStatus.REJECTED
  );
}
...
export async function GET(req: Request) {
  try {
    ...
    if (nicheGroup) profileFilters.nicheGroup = nicheGroup;

    if (country) {
      profileFilters.country = { contains: country, mode: "insensitive" };
    }

    if (isWorkMode(workMode)) {
      profileFilters.workMode = workMode;
    }

    if (isApprovalStatus(approvalStatus)) {
      profileFilters.approvalStatus = approvalStatus;
    }
    ...
    const withMeta = await Promise.all(
      creators.map(async (creator) => {
        ...
        const emailConfirmed = !!creator.emailConfirmedAt;
    ...
    console.error("api/staff/creators GET error:", e);
    return NextResponse.json(
